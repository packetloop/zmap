...
static void *start_send(void *arg)
{
	...
	log_debug("zmap", "Pinning a send thread to core %u, thread(%d)", s->cpu, syscall(SYS_gettid));
	set_cpu(s->cpu);
...
static void *start_recv(void *arg)
{
	...
	log_debug("zmap", "Pinning receive thread to core %u, thread(%d)", r->cpu, syscall(SYS_gettid));
	set_cpu(r->cpu);
...
static void *start_mon(void *arg)
{
	...
	log_debug("zmap", "Pinning monitor thread to core %u, thread(%d)", mon_arg->cpu, syscall(SYS_gettid));
	set_cpu(mon_arg->cpu);
